...
class HubAttachedIO(collections.namedtuple('HubAttachedIO', ('port', 'event', 'io_type'))):
    @classmethod
    def from_bytes(cls, value):
        port, event = struct.unpack('BB', value[:2])
        event = HubAttachedIOEvent(event)
        if event in (HubAttachedIOEvent.AttachedIO, HubAttachedIOEvent.AttachedVirtualIO):
